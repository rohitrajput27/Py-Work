...
def prepareRequest(accountId, transactionId):
    body = (
            '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:acc="http://www.charter.com/solo/synch/account">'
   +'<soapenv:Header>'
    +'<acc:charterHeaderInputInfo>'
         
     +'<clientCallingService>EDGECACHERESEND</clientCallingService>'
   ...
   +'</soapenv:Header>'
   +'<soapenv:Body>'
    +'<acc:sendEdgeCacheMessageByAccountIdRequest>'
...
with open(inputFile, "r") as inputData, open(outputFile, "w+") as outputData:
    outputData.write("SoloAccountID,TransactionId,Status\n")
    ...
    for line in inputData:
        ...
        body = prepareRequest(accountId=line, transactionId=transactionId)
        try:
            response = requests.post(url, data=body, headers=headers, verify=False)
#             print(response.content)
            if parse(response.content) == transactionId:
                sucess = sucess + 1
